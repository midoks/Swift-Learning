...
MusicListTableViewDelegate, MusicAlertPageViewDelegate,UIActionSheetDelegate{
    ...
    override func viewDidLoad() {
        ...
        //获取view的宽度
        let viewW = self.view.bounds.size.width/2;
        
        //展示
        let showText = UILabel(frame: CGRectMake(viewW, 50, self.view.bounds.size.width, 100))
        ...
        //建立button
        clickButton = UIButton(type: UIButtonType.System);
        ...
        //排列播放表
        let listView = MusicListTableView(frame: CGRectMake(0, 0, self.view.bounds.size.width, 60))
        ...
        //播放的方式
        let playChooseItem = ["单曲播放","循序播放","随机播放"]
    ...
    {
        let name = clickButton.titleLabel?.text
        if(name == "吹"){
            //NSLog("%@", "song")
            RecondPay(0)
    ...
    func audio_setting(pos:NSInteger){
        let setting = NSMutableDictionary()
        //设置录音格式  AVFormatIDKey==kAudioFormatLinearPCM
        setting.setValue(Int(kAudioFormatMPEG4AAC), forKey: AVFormatIDKey)
        ...
        //setting.dictionaryWithValuesForKeys(AVNumberOfChannelsKey)
        ...
        let urlt:NSURL = NSBundle.mainBundle().sharedSupportURL!
        let fm = NSFileManager()
        
        if(fm.fileExistsAtPath(urlt.path!)){
            NSLog("文件存在")
            do {
                try fm.removeItemAtPath(NSString(format:"%@/md_%d.aac", urlt.path!, pos) as String)
            } catch _ {
            }
        }else{
            NSLog("文件不存在")
            NSLog("%@", urlt.path!)
            do {
                try fm.createDirectoryAtPath(urlt.path!, withIntermediateDirectories: true, attributes: nil)
            } catch _ {
            }
        }
        
        let stt = NSString(format:"%@/md_%d.aac", urlt.path!, pos)
        let url:NSURL! = NSURL(fileURLWithPath: stt as String)
        //var error:NSError?
        
        
        recorder = try! AVAudioRecorder(URL:url, settings: [
            AVFormatIDKey:Int(kAudioFormatMPEG4AAC),
            AVSampleRateKey:NSNumber(float:44100),
    ...
    func loadSound2(abspath: NSString){
        let urlt:NSURL = NSBundle.mainBundle().sharedSupportURL!
        let stt = NSString(format:"%@/%@", urlt.path!, abspath)
        let url:NSURL! = NSURL(fileURLWithPath: stt as String)
        let data = NSData(contentsOfURL: url!)
    ...
    func RecondPay(pos: NSInteger){
        ...
        if(fm.fileExistsAtPath(stt as String)){
            let stt0 = NSString(format: "md_%d.aac", pos)
            loadSound2(stt0)
            sound(stt0)
        }else{
            let stt2 = NSString(format: "sc_%d", pos);
    ...
    func RecondSound(pos:NSInteger){
        let sound = MusicAlertPageView(frame: CGRectMake(0, 0, self.view.bounds.size.width, self.view.frame.height))
        sound.start()
        sound.delegate = self
    ...
    func RecondDelete(pos:NSInteger){
        ...
        if(fm.fileExistsAtPath(stt as String)){
            do {
                try fm.removeItemAtPath(stt as String)
            } catch _ {
            }
        }
        //        NSLog("url:%@", stt)
