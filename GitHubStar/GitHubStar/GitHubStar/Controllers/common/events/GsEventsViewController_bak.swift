        ...
        GitHubApi.instance.urlGet(_fixedUrl) { (data, response, error) -> Void in
            ...
            if data != nil {
                ...
                if rep.allHeaderFields["Link"] != nil {
                    self.pageInfo  = self.gitParseLink(rep.allHeaderFields["Link"] as! String)
                }
            }
...
extension GsEventsViewController:GsListViewDelegate {
    ...
    func listTableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.authorIcon.MDCacheImage(indexData["actor"]["avatar_url"].stringValue, defaultImage: "avatar_default")
        cell.eventTime.text = gitNowBeforeTime(indexData["created_at"].stringValue)
        ...
        if indexData["type"].stringValue == "CommitCommentEvent" {
            ...
            var content = author
            content += " " + actionName(indexData["type"].stringValue)
            
            let ref = indexData["payload"]["ref"].stringValue
            let refs = ref.componentsSeparatedByString("/")
            content += " " + refs[refs.count - 1]
            content += " " + repo
            cell.actionContent.text = content
            
            
            for i in indexData["payload"]["commits"] {
                let v = i.1
                let msg = v["sha"].stringValue.substring(0, end: 5) + " - " + v["message"].stringValue.componentsSeparatedByString("\n\n")[0]
                let labelList = UILabel()
                labelList.text = msg
                cell.nextList.append(labelList)
            }
            ...
            cell.actionContent.addLinkToURL(NSURL(string: "scheme://?type=1&business_id=2"), withRange: NSRange(location: 0, length: author.length))
        }
